...
//
// אחסון: כמו visitStats — קטגוריה פנימית (__nc_ad_stats) באוסף ה-items
...
const STATS_CATEGORY = '__nc_ad_stats';
...
async function loadStats(): Promise<{ id: string | null; store: RawStore }> {
    try {
        ...
        if (!(e instanceof StrapiContentTypeError)) {
            console.error('[neighborhoods] adStats load failed:', e);
        }
    ...
    try {
        ...
        if (id) {
            ...
            const res = await strapiPost<{ data: StrapiItem }>('/api/items', {
                data: {
                    label: 'neighborhoods-ad-stats',
                    category: STATS_CATEGORY,
                    description: '[SYSTEM] מדדי פרסומות — ועדי שכונות',
                    icon: '📊',
        ...
        console.error('[neighborhoods] adStats flush failed:', e);
    } finally {
        flushing = false;
    }
}
