<script lang="ts">
    ...
    // ===== אזהרת מקום תפוס בבורר "⇄ העבר" =====
    // מי תופסת כל מקום בלוח — גם מושהית/פגה שומרת את המקום שלה
    let slotOccupants = $derived(new Map(
        approved
            .filter((a: any) => typeof a.slot === 'number')
            .map((a: any) => [a.slot as number, { id: a.id as string, title: a.title as string }])
    ));
    ...
    // ===== תצוגה מקדימה של הכרטיס כפי שהוא באמת מוצג בטור הפרסומות =====
    // ריחוף על כותרת פרסומת מאושרת (דסקטופ) או הקשה עליה (נייד/דסקטופ)
    let approvedById = $derived(new Map(approved.map((a: any) => [a.id as string, a] as const)));
    ...
    function openHoverPreview(e: MouseEvent, id: string) {
        ...
        // הכרטיס צף לצד הכותרת, מוצמד לגבולות המסך (fixed) — כדי ששום
        // overflow של הכרטיסים לא יחתוך אותו
                    ...
                    <div class="occ-row">
                        <div class="occ-head">
                            ...
                            <button type="button" class="occ-title title-preview-btn"
                                    onmouseenter={(e) => openHoverPreview(e, a.id)}
                                    onmouseleave={() => hoverPreview = null}
                                    onclick={() => { hoverPreview = null; modalPreviewId = a.id; }}
                ...
                <div class="ad-card-body">
                    <div class="ad-card-head">
                        {#if ad.status === 'approved'}
                            <!-- ריחוף = תצוגה מקדימה צפה של הכרטיס; הקשה = מודאל -->
                            <h2>
                                <button type="button" class="title-preview-btn"
                                        onmouseenter={(e) => openHoverPreview(e, ad.id)}
                                        onmouseleave={() => hoverPreview = null}
                                        onclick={() => { hoverPreview = null; modalPreviewId = ad.id; }}
                                        title="תצוגה מקדימה של הפרסומת כפי שהיא מוצגת באתר">
                                    {ad.title}
                                </button>
                            </h2>
                        ...
                        <div class="slot-row">
                            ...
                            <form method="POST" action="?/setSlot" use:enhance class="slot-jump-form">
                                ...
                                <select name="slot" class="duration-select" aria-label="מספר מקום בלוח"
                                    ...
                                    {#each slotOptions as n (n)}
                                        ...
                                        <!-- רקע לבן + טקסט כהה חובה: רקע כהה נבלע בהדגשת הבחירה של המערכת;
                                             מקום תפוס — רקע אדמדם + שם הפרסומת שיושבת בו -->
                                ...
                                {#if slotWarning[ad.id]}
                                    <span class="slot-warn">⚠ {slotWarning[ad.id]}</span>
                                {/if}
                            </form>
                        </div>
                    {/if}
...
<style>
    ...
    /* אזהרה חיה כשנבחר בבורר מקום שתפוס ע"י פרסומת אחרת */
    .slot-warn {
        font-size: 0.68rem;
        font-weight: 700;
        color: #fcd34d;
    ...
    /* כותרת שהיא כפתור תצוגה מקדימה — נראית כמו הכותרת, עם רמז קו מנוקד */
    .title-preview-btn {
        background: none;
        border: none;
        padding: 0;
