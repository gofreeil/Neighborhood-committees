<script lang="ts">
    ...
    const mailto =
        `mailto:${email}` +
        `?subject=${encodeURIComponent('פרסום בוועדי שכונות')}` +
        `&body=${encodeURIComponent(
            'שלום,\nאשמח לפרסם באתר ועדי שכונות.\n\n' +
            'שם העסק / הארגון:\n' +
            'טלפון לחזרה:\n' +
            'המסלול המבוקש:\n'
        )}`;
...
<div class="mx-auto max-w-3xl px-4 py-8 md:py-12" dir="rtl">
    ...
    <div class="p-6 md:p-10 text-center">
        ...
        <p class="text-gray-300 text-base md:text-lg leading-relaxed max-w-xl mx-auto">
            תושבים ופעילי ועדים נכנסים לכאן כדי לקדם את השכונה שלהם.
            המודעה שלכם פוגשת אותם בדיוק ברגעי העשייה — וההכנסות מהפרסום
    ...
    <div class="grid md:grid-cols-2">
        {#each placements as p, i (p.title)}
            <div class="relative p-6 flex flex-col">
                ...
                <p class="text-sm text-blue-300 font-bold mb-4">{p.where}</p>
                <ul class="space-y-2">
    ...
    <div class="p-6 md:p-8 text-center">
        ...
        <ol class="text-right max-w-md mx-auto space-y-4 mb-8">
            <li class="flex items-start gap-3">
                <span class="w-7 h-7 rounded-full bg-blue-500 text-white text-sm font-black flex items-center justify-center flex-shrink-0">1</span>
                <span class="text-gray-200 text-sm md:text-base leading-relaxed">מעצבים את הפרסומת בקלות ישירות על האתר.</span>
            </li>
            <li class="flex items-start gap-3">
                <span class="w-7 h-7 rounded-full bg-blue-500 text-white text-sm font-black flex items-center justify-center flex-shrink-0">2</span>
                <span class="text-gray-200 text-sm md:text-base leading-relaxed">בוחרים תקופת פרסום ומשלמים מראש.</span>
            </li>
            <li class="flex items-start gap-3">
                <span class="w-7 h-7 rounded-full bg-blue-500 text-white text-sm font-black flex items-center justify-center flex-shrink-0">3</span>
        ...
        <a
            ...
            class="inline-block rounded-2xl bg-gradient-to-r from-blue-500 to-purple-600 px-8 py-4 font-black text-white text-lg shadow-xl transition hover:opacity-90 hover:scale-[1.02]"
        >
            🎨 לעיצוב הפרסומת
        </a>
        <p class="mt-4 text-sm text-gray-300">
            כבר יש לכם פרסומת?
            <a href="/advertise/manage" class="font-bold text-blue-300 underline hover:text-blue-200 transition-colors">
        ...
        <ul class="max-w-md mx-auto space-y-2">
            {#each adPlans as plan (plan.days)}
                <li class="flex items-baseline justify-between gap-3 rounded-xl bg-[#0b1120] border border-[#2c3a55] px-4 py-3">
                    ...
                    <span class="text-blue-300 font-black text-base md:text-lg whitespace-nowrap">{plan.price} ₪</span>
                </li>
            {/each}
        </ul>
