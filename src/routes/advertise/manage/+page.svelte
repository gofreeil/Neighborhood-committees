...
<svelte:head><title>הנכסים שלי | ועדי שכונות | יוצאים לחירות</title></svelte:head>
...
<section class="mx-auto max-w-4xl px-3 py-6 md:px-4" dir="rtl">
    <div class="mb-5 flex flex-wrap items-end justify-between gap-3">
        <div>
            ...
            <p class="mt-2 inline-block rounded-full border border-[#2c3a55] bg-[#172136] px-3 py-1 text-xs font-semibold text-gray-100 shadow-md">
                {data.ads.length} פרסומות בחשבון {data.user.email}
            </p>
        </div>
        <a href="/advertise" class="rounded-full border border-[#2c3a55] bg-[#172136] px-3.5 py-1.5 text-sm font-bold text-gray-100 shadow-md transition-colors hover:bg-[#223252] hover:text-white">
            ➕ פרסומת חדשה
        </a>
    </div>
    ...
    {:else if data.ads.length === 0}
        <div class="rounded-2xl border border-[#2c3a55] bg-[#0f1626] px-6 py-12 text-center shadow-lg">
            ...
            <a href="/advertise" class="mt-5 inline-block rounded-xl bg-gradient-to-r from-blue-500 to-purple-600 px-6 py-3 font-bold text-white transition hover:opacity-90">
                לפרסום באתר ←
            </a>
        </div>
    {:else}
        <div class="flex flex-col gap-4">
            {#each data.ads as ad (ad.id)}
                ...
                <article class="overflow-hidden rounded-2xl border border-[#2c3a55] bg-[#0f1626] transition-all hover:border-[#40527a] hover:bg-[#16203a]">
                    <a href="/advertise/manage/{ad.id}" class="flex items-stretch gap-4 p-4">
                        ...
                        <div
                            class="relative flex min-h-[130px] w-2/5 max-w-[190px] flex-shrink-0 items-center justify-center self-stretch overflow-hidden rounded-xl border border-[#2c3a55]"
