<script lang="ts">
    ...
    const TONE: Record<string, string> = {
        ...
        slate: 'border-[#2c3a55] bg-[#172136] text-gray-300',
    };
    ...
    const LS_KEY = 'nc_ad_builder_draft_v1';
    const EDIT_KEY = 'nc_ad_edit_id';
    let editError = $state('');
    ...
    const renewWa = $derived(
        ...
        encodeURIComponent(`שלום! אני רוצה לחדש את הפרסום של "${ad.title}" בוועדי שכונות 🙂`)
    );
</script>

<svelte:head><title>ניהול הפרסומת {ad.title} | ועדי שכונות | יוצאים לחירות</title></svelte:head>
...
<section class="mx-auto max-w-3xl px-3 py-6 md:px-4" dir="rtl">
    ...
    <header class="mt-4 rounded-2xl border border-[#2c3a55] bg-[#0f1626] p-4 md:p-5">
        <div class="flex flex-col gap-4 md:flex-row md:items-start">
            <div class="min-w-0 flex-1">
                ...
                <dl class="mt-3.5 grid grid-cols-1 gap-x-6 gap-y-1.5 border-t border-[#2c3a55] pt-3.5 text-sm sm:grid-cols-2">
                ...
                <div class="mt-4 flex flex-wrap gap-2">
                    <button type="button" onclick={editInBuilder}
                        class="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-blue-500 to-purple-600 px-4 py-2 text-sm font-bold text-white transition hover:opacity-90">
                        ✏️ ערוך את הפרסומת
                    </button>
                    {#if ad.status === 'approved'}
                        <a href="/ads/{ad.id}" target="_blank" rel="noopener"
                            class="inline-flex items-center gap-2 rounded-xl bg-[#172136] px-4 py-2 text-sm font-bold text-white transition-colors hover:bg-[#223252]">
                            🔗 לדף הנחיתה
                        </a>
                    {/if}
                </div>
    ...
    <!-- מדדים -->
    <section class="mt-4 rounded-2xl border border-[#2c3a55] bg-[#0f1626] p-4 md:p-5">
        ...
        <div class="mt-4 grid grid-cols-2 gap-3 md:grid-cols-4">
            {#snippet stat(label: string, value: number, hint: string, accent: string)}
                <div class="rounded-xl border border-[#2c3a55] bg-[#0b1120] px-3 py-3 text-center">
        ...
        {#if totals.impressions === 0}
            <p class="mt-4 rounded-xl border border-[#2c3a55] bg-[#0b1120] px-3 py-2.5 text-sm text-gray-300">
                {#if ad.status === 'approved'}
    ...
    <section class="mt-4 rounded-2xl border border-[#2c3a55] bg-[#0f1626] p-4 md:p-5">
        ...
        <ul class="mt-3 flex flex-col gap-2 text-sm text-gray-300">
            <li class="rounded-xl border border-[#2c3a55] bg-[#0b1120] px-3 py-2">
