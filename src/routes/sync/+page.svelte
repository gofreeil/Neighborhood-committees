    ...
    {#each events as e}
        <div class="rounded-2xl bg-white/5 border border-white/10 p-4 flex items-center gap-4 flex-wrap">
            ...
            <div class="flex-1 min-w-0">
                ...
                <p class="text-xs text-gray-400 mt-1">📍 {e.location} · {e.attending} משתתפים מאושרים</p>
            </div>
            <button class="px-4 py-2 rounded-lg bg-gradient-to-r from-teal-600 to-emerald-600 text-white font-bold text-sm">
                אשר השתתפות
            </button>
        </div>
    {/each}
